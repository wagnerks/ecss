...
namespace RegistryTests {
	...
	TEST(Registry, AddAndgetComponent) {
	    Registry registry;
	    auto entity = registry.takeEntity();
	    registry.addComponent<Position>(entity, { 10, 20 });
	...
	TEST(Registry, HasComponentWorks) {
	    ...
	    registry.addComponent<Velocity>(entity, 1, 2 );
	...
	TEST(Registry, AddMultipleComponents) {
	    Registry registry;
	    auto entity = registry.takeEntity();
	    registry.addComponent<Position>(entity, 1, 2 );
	    registry.addComponent<Velocity>(entity, 3, 4 );
	...
	TEST(Registry, ComponentOverwrite) {
	    Registry registry;
	    auto entity = registry.takeEntity();
	    registry.addComponent<Velocity>(entity, 1, 1);
	    registry.addComponent<Velocity>(entity, 2, 2); // Overwrite
	    auto* vel = registry.getComponent<Velocity>(entity);
	    ASSERT_NE(vel, nullptr);
	    EXPECT_FLOAT_EQ(vel->dx, 2);
	}

	TEST(Registry, getComponentNonexistentReturnsNull) {
	    Registry registry;
	    auto entity = registry.takeEntity();
	    EXPECT_EQ(registry.getComponent<Health>(entity), nullptr);
	}
	...
	TEST(Registry, ComponentStorageIsIsolatedPerEntity) {
	    ...
	    EXPECT_EQ(registry.getComponent<Health>(e1)->value, 99);
	    EXPECT_EQ(registry.getComponent<Health>(e2)->value, 42);
	}
	...
	TEST(Registry, OverwriteWithDifferentComponents) {
	    ...
	    registry.addComponent<Position>(entity, 1, 2);
	    registry.addComponent<Velocity>(entity, 3, 4);
	    registry.addComponent<Position>(entity, 7, 8);
	    EXPECT_EQ(registry.getComponent<Position>(entity)->x, 7);
	    EXPECT_EQ(registry.getComponent<Velocity>(entity)->dx, 3);
	}
	...
	TEST(Registry, MassEntityAndComponentAddRemove) {
	    ...
	    int alive = 0;
	    for (int i = 0; i < N; ++i) {
	        if (registry.getComponent<Health>(ids[i]))
	            ++alive;
	    }
	    EXPECT_EQ(alive, N / 2);
	}
	...
	TEST(Registry, ReAddComponentAfterRemove) {
	    ...
	    EXPECT_EQ(registry.getComponent<Health>(e), nullptr);
	    registry.addComponent<Health>(e, 20);
	    EXPECT_EQ(registry.getComponent<Health>(e)->value, 20);
	}
	...
	TEST(Registry, ForEachSkipsEntitiesWithoutAllComponents) {
	    ...
	    registry.addComponent<Position>(a, 1, 2);
	    registry.addComponent<Velocity>(a, 3, 4);
	    registry.addComponent<Position>(b, 5, 6); // b без Velocity
	...
	TEST(Registry, DifferentComponentTypesDoNotInterfere) {
	    ecss::Registry registry;
	    auto e = registry.takeEntity();
	    registry.addComponent<Position>(e, 1, 2);
	    EXPECT_EQ(registry.getComponent<Health>(e), nullptr);
	    registry.addComponent<Health>(e, 42);
	    EXPECT_EQ(registry.getComponent<Position>(e)->x, 1);
	    EXPECT_EQ(registry.getComponent<Health>(e)->value, 42);
	}
	...
	TEST(Registry, InsertMoveOnlyComponent) {
	    ...
	    auto* ptr = registry.getComponent<MoveOnly>(e);
	...
	TEST(Registry, ReserveCapacity) {
	    ...
	    auto e = registry.takeEntity();
	    registry.addComponent<Position>(e, 5, 6);
	    EXPECT_NE(registry.getComponent<Position>(e), nullptr);
	}
	...
	TEST(Registry, ThreadedAddRemoveStress) {
	    ...
	    for (int t = 0; t < threadCount; ++t) {
	        threads.emplace_back([&, t] {
	            for (int i = 0; i < N; ++i) {
	                ...
	                registry.addComponent<Position>(e, idx, -idx);
	            }
	        });
	    }
	    ...
	    for (int i = 0; i < threadCount * N; ++i) {
	        ...
	        if (registry.getComponent<Position>(ids[i]))
	            ++countP;
	        if (registry.getComponent<Health>(ids[i]))
	            ++countH;
	    }
	...
	TEST(Registry, ClearRemovesAllEntitiesAndComponents) {
	    ...
	    registry.clear();
	    EXPECT_EQ(registry.getComponent<Health>(e), nullptr);
	    EXPECT_EQ(registry.getComponent<Position>(e), nullptr);
	...
	TEST(Registry, InitCustomComponentContainer) {
	    ...
	    EXPECT_EQ(registry.getComponent<A>(e)->a, 10);
	    EXPECT_FLOAT_EQ(registry.getComponent<B>(e)->b, 2.5f);
	}
	...
	TEST(Registry, ReuseAfterClear) {
	    ...
	    registry.clear();
	    auto e2 = registry.takeEntity();
	    registry.addComponent<Position>(e2, 7, 8);
	    EXPECT_EQ(registry.getComponent<Position>(e2)->x, 7);
	}
	...
	TEST(Registry, ReserveAfterDeleteAndAdd) {
	    ...
	    EXPECT_EQ(registry.getComponent<Health>(e2)->value, 2);
	}
	namespace Stress
	{
		...
		TEST(Registry_STRESS, StressTestCreatingAndIteratingOneSectorRanges) {
			ecss::Registry registry;
			constexpr size_t size = 60000000;
			...
			int counter = 0;
			auto t2 = std::chrono::high_resolution_clock::now();
			for (auto [e, hel, vel] : registry.forEach<Health, Velocity>(EntitiesRanges{ std::vector<EntitiesRanges::range>{{0,size}} })) {
				hel->value += e;
				vel->dx += e;
				vel->dy += hel->value;
				counter++;
		...
		TEST(Registry_STRESS, StressTestCreatingAndIteratingNotOneSectorNotReserved) {
			ecss::Registry registry;
		...
		TEST(Registry_STRESS, StressTestCreatingAndIteratingOneSectorNotReserved) {
			ecss::Registry registry;
			constexpr size_t size = 10000000;
			constexpr size_t itCount = 10;
	...
	TEST(Registry, AddRemoveComponentMultipleTimes) {
	    ...
	    EXPECT_TRUE(registry.hasComponent<Health>(e));
	    EXPECT_EQ(registry.getComponent<Health>(e)->value, 321);
	}
	...
	TEST(Registry, ThreadSafeGetComponent) {
	    ...
	    std::thread t1([&]() {
	        while (!stop.load()) {
	            auto ptr = registry.getComponent<Health>(e);
	...
	TEST(Registry, AddNoDefaultCtor) {
	    ...
	    ASSERT_NE(reg.getComponent<NoDefaultCtor>(e), nullptr);
	    EXPECT_EQ(reg.getComponent<NoDefaultCtor>(e)->x, 42);
	}
	...
	TEST(Registry, MoveOnlyComponentWorks) {
	    ...
	    auto* ptr = reg.getComponent<MoveOnly>(e);
	    ASSERT_NE(ptr, nullptr);
	...
	//    auto* ptr = reg.getComponent<CopyOnly>(e);
	//    ASSERT_NE(ptr, nullptr);
	//    EXPECT_EQ(ptr->v, 555);
	//}
	...
	TEST(Registry, MultipleEntitiesSameComponentType) {
	    ...
	    for (int i = 0; i < 10; ++i) {
	        EXPECT_EQ(reg.getComponent<Health>(ids[i])->value, i * 10);
	    }
	}
	...
	TEST(Registry, MassAddRemoveAndIterate) {
	    ...
	    for (int i = 0; i < N; ++i) {
	        auto e = reg.takeEntity();
	        reg.addComponent<Position>(e, i, -i);
	        ids.push_back(e);
	    }
	    for (int i = 0; i < N; i += 2)
	    ...
	    for (int t = 0; t < threads; ++t) {
	        thrs.emplace_back([&, t] {
	            for (int i = 0; i < N; ++i) {
	                ...
	                auto e = reg.takeEntity();
	                ids[idx] = e;
	                reg.addComponent<Position>(e, idx, -idx);
	            }
	        });
	    }
	    ...
	    for (int t = 0; t < threads; ++t) {
	        reads.emplace_back([&, t] {
	            for (int i = t * N; i < (t + 1) * N; ++i) {
	                auto* pos = reg.getComponent<Position>(ids[i]);
	                if (pos) found++;
	            }
	        });
	    }
	...
	TEST(Registry, RegisterArrayMultipleTypes) {
	    ...
	    auto e = reg.takeEntity();
	    reg.addComponent<Position>(e, 1, 2);
	    reg.addComponent<Velocity>(e, 3, 4);
	    reg.addComponent<Health>(e, 5);

	    EXPECT_EQ(reg.getComponent<Position>(e)->x, 1);
	    EXPECT_EQ(reg.getComponent<Velocity>(e)->dx, 3);
	    EXPECT_EQ(reg.getComponent<Health>(e)->value, 5);
	}
	...
	TEST(Registry, ParallelAddRemove) {
	    ...
	    for (int i = 0; i < threads * N; ++i)
	        if (reg.getComponent<Health>(ids[i])) alive++;
	...
	TEST(Registry, StressDestroyAll) {
	    ...
	    for (auto e : ids)
	        EXPECT_EQ(reg.getComponent<Health>(e), nullptr);
	}
	...
	TEST(Registry, DestroyAllComponentsOnEntity) {
	    ...
	    auto e = reg.takeEntity();
	    reg.addComponent<Health>(e, 1);
	    reg.addComponent<Position>(e, 2, 3);
	    reg.destroyComponents(e);
	    EXPECT_EQ(reg.getComponent<Health>(e), nullptr);
	    EXPECT_EQ(reg.getComponent<Position>(e), nullptr);
	}
	...
	TEST(Registry, CopyArrayToRegistry) {
	    ...
	    EXPECT_EQ(reg2.getComponent<Health>(e)->value, 42);
	}
	...
	TEST(Registry, ForEachOrderIndependence) {
	    ...
	    for (int i = 0; i < size; ++i) {
	        ...
	        reg.addComponent<Health>(e, i);
	        reg.addComponent<Position>(e, i, -i);
	        reg.addComponent<Velocity>(e, 10, -10);
	    }
	    int cnt = 0;
	...
	TEST(Registry, MoveCopyStress) {
	    ...
	    for (int i = 0; i < N; ++i) {
	        EXPECT_EQ(reg.getComponent<MoveOnly>(ids[i])->val, i);
	        //EXPECT_EQ(reg.getComponent<CopyOnly>(ids[i])->v, i); //this code should not compile
	    }
	}
	...
	TEST(Registry, ParallelMoveOnlyInsertGetErase) {
	    ...
	    int found = 0;
	    for (int i = 0; i < T * N; ++i)
	        if (reg.getComponent<MoveOnly>(ids[i]))
	            found++;
	    EXPECT_GE(found, 0);
	}
	...
	TEST(ChunksManager, ConcurrentAccessBugRepro) {
		...
		std::thread writer([&] {
			for (int i = 0; i < 10'000'00 && running; ++i) {
				auto ent = reg.takeEntity();
	...
	TEST(Registry_STRESS, ConcurrentAddAndIterate_TransformMatComp_RawMat4)
	{
		...
		// Поток: копирует TransformMatComp из source в target
		std::thread writer([&]() {
			for (int k = 0; k < 10; k++) {
				...
				for (auto id : shuffledIds) {
					target.addComponent<TransformMatComp>(id, source.getComponent<TransformMatComp>(id)->mTransform);
		...
		// Поток: итерирует target и проверяет, не попала ли порча
		std::thread reader([&]() {
			while (!stop) {
	...
	TEST(Registry, ParallelAddingToTheRandomPlacesAndIteration) {
		...
		threads.emplace_back([&]()
		{
			...
			for (auto i = 0; i < count; i++) {
				...
				reg.addComponent<Velocity>(randomValue, randomValue);
	...
	TEST(Registry, IteratinNotFromStart) {
		...
		int i = 525;
		for (auto [ent, vel] : reg.forEach<Velocity>(EntitiesRanges{ std::vector<EntitiesRanges::range>{{525,1000000}}})) {
			EXPECT_EQ(ent, i++);
		}
