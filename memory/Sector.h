...
namespace ecss::Memory {
	...
	public:
		template<typename T>
		inline constexpr static size_t TypeId() { return TypeIdImpl<std::remove_const_t<std::remove_pointer_t<T>>>(); }
	...
	inline void initLayoutData(LayoutData& data, uint16_t& index, uint32_t offset) {
		...
		data.functionTable.copy = [](void* dest, void* src) { new(dest)U(*static_cast<U*>(src)); };
		data.functionTable.destructor = [](void* src) { static_cast<U*>(src)->~U(); };
	}

	template<typename... U>
	inline void initLayoutData(LayoutData* dataArray, const std::array<uint32_t, sizeof...(U)>& offsets) {
		uint16_t counter = 0;
		(initLayoutData<std::remove_const_t<std::remove_pointer_t<U>>>(dataArray[counter], counter, offsets[counter]), ...);
	}
...
#define INITIALIZE_COMMON_META_DATA(...)																					\
	...
	private:
		static constexpr size_t typeIds[count] = { TypeId<Types>()... };
	...
	*/
	struct Sector {
		Sector(SectorId id) : id(id), isAliveData(0) {}
