...
|  FORCE\_INLINE void | [**advanceToId**](#function-advancetoid) (SectorId minId) <br>_Jump to the first alive sector whose id is &gt;=_ `minId` _(or end)._ |
|  FORCE\_INLINE void | [**becomeEnd**](#function-becomeend) () noexcept<br> |
...
_Jump to the first alive sector whose id is &gt;=_ `minId` _(or end)._
```C++
inline FORCE_INLINE void ecss::Memory::SectorsArray::IteratorAlive::advanceToId (
    SectorId minId
) 
```
...
```C++
inline FORCE_INLINE void ecss::Memory::SectorsArray::IteratorAlive::becomeEnd () noexcept
```
