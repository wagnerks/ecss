...
|  void | [**insertBulk**](#function-insertbulk) (It first, It last) <br>_Append-only bulk insert of component T for a sorted batch of (entity, value) pairs._  |
...
_Append-only bulk insert of component T for a sorted batch of (entity, value) pairs._ 
```C++
...
* `first` Range of pairs. Ids MUST be strictly ascending and greater than any id already present in T's array (pure append; no overwrite, no middle insert). 



**Note:**

Amortizes per-element overhead (existence check, insert-position search, view publish) and, in the TS build, the write lock and pin wait across the whole range. 
...
* `ticks` Number of [**tick()**](classecss_1_1Registry.md#function-tick) calls before memory is freed 
...
Only available in thread-safe mode. In non-thread-safe mode, memory is freed immediately during reallocation (no deferred reclamation).
...
Total number of memory blocks freed across all arrays 
...
Automatically frees retired memory that has passed the grace period (default 3 ticks). @thread\_safety Internally synchronized. 
