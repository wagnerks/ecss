...
|   | [**RangesCursor**](#function-rangescursor-22) (const [**ChunksAllocator**](structecss_1_1Memory_1_1ChunksAllocator.md) \* alloc, const [**Ranges**](structecss_1_1Ranges.md)&lt; SectorId &gt; & ranges, size\_t size) <br> |
...
@thread\_safety Thread-confined. Applies to every member of this cursor. It belongs to the thread that made it and holds no lock; any number of threads may each walk their own. Built from the chunk snapshot the array published, so stepping stays valid while chunks are added or returned  but not across a compaction, which moves the sectors it is pointing at. That is what the owning view's structural hold prevents. 
