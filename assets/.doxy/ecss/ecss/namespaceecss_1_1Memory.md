...
| struct | [**RetireBin**](structecss_1_1Memory_1_1RetireBin.md) <br>_Deferred memory reclamation bin with grace period support._  |
| struct | [**SectorLayoutMeta**](structecss_1_1Memory_1_1SectorLayoutMeta.md) <br>_Immutable description of one sector's component layout._  |
...
| struct | [**StructuralHold**](structecss_1_1Memory_1_1StructuralHold.md) <br>_RAII structural hold: while one is alive, no sector in the array may be relocated._  |
...
Uses address of static variable as unique type identifier. This is resolved at compile/link time - zero runtime overhead. IDs are stable across the program lifetime but NOT dense (not sequential). @thread\_safety Internally synchronized. The address of a static, resolved at link time. Nothing is written at runtime. 
...
@thread\_safety Internally synchronized. Pure pointer arithmetic on its arguments; no shared state is touched. 
