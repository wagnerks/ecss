...
_Non-thread-safe sparse map (simple vector)_ 
...
| Type | Name |
| ---: | :--- |
|  std::vector&lt; [**SlotInfo**](structecss_1_1Memory_1_1detail_1_1SlotInfo.md) &gt; | [**sparse**](#variable-sparse)  <br> |
...
```C++
std::vector<SlotInfo> ecss::Memory::detail::SparseMap< false >::sparse;
```
...
```C++
inline FORCE_INLINE SlotInfo ecss::Memory::detail::SparseMap< false >::find (
    SectorId id
) const
```
...
```C++
inline FORCE_INLINE uint32_t ecss::Memory::detail::SparseMap< false >::findLinearIdx (
    SectorId id
) const
```
...
```C++
inline FORCE_INLINE SlotInfo ecss::Memory::detail::SparseMap< false >::get (
    SectorId id
) const
```
...
```C++
inline FORCE_INLINE SlotInfo & ecss::Memory::detail::SparseMap< false >::operator[] (
    SectorId id
) 
```
...
```C++
inline FORCE_INLINE const SlotInfo & ecss::Memory::detail::SparseMap< false >::operator[] (
    SectorId id
) const
```
